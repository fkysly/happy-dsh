...
// credential is absent, the credential step takes the shipped modal chrome as
// the only onboarding step, and the inline key write lands in an isolated
...
describe.skipIf(MODE === 'record')('web e2e: first-run DeepSeek credential setup', () => {
  ...
  it('stores a key write-only and observes configured state without restarting', async () => {
    ...
    // This build registers no internal-testing notice, so the credential step
    // is itself the first-run takeover.
    const credentialStep = page.getByRole('dialog', { name: '添加一个 API Key 开始使用' })
    await credentialStep.waitFor({ timeout: 15_000 })
    expect(await page.locator('#root').evaluate(root => (root as HTMLElement).inert)).toBe(true)

    // The takeover holds across a reload while the credential is still absent.
    ...
    // A configured provider stays complete across a reload, and no
    // onboarding step returns.
  ...
  it('never paints the takeover chrome on a configured reload, even with the settings join held open', async () => {
    ...
    // Literal, not a mirrored constant: this build ships no notice, so a test
    // that reads its copy would be asserting against nothing.
