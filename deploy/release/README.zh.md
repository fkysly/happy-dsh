...
4. **那个提交在 `master` 上的九个闸没有全绿。** 这条是真正管用的那条。workflow 读该提交的
   check runs，只保留在 `master` 上跑过的那些，然后要求九个名字（与分支保护要求的同一批）
   各有一个成功的运行。只在特性分支上绿过的提交，会被拒绝。

check runs 只保留 90 天，所以对更老的提交重新发版没法用这个办法证明。`allow-ungated` 是
