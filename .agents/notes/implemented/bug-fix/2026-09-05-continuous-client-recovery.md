...
A page hidden longer than `resumeAfterHiddenMs` (30 seconds by default) reconnects when it becomes visible again. This covers the loss no retry schedule can observe: a suspended page has its timers frozen and can have its socket half-closed without an event, so the schedule never starts, while the Client keeps the state it held when the page was hidden. A return within the threshold reconnects nothing, which keeps an ordinary app switch from replacing a healthy stream. The threshold is a Host-set `recovery` field like the others, so a deployment whose clients suspend differently can retune it.
