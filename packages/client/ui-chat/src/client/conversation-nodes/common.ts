 ...
 * turn-end notices sit between a closing Assistant and the turn-tail so the
...
export const CHAT_SYNTHETIC_SEQ_OFFSETS = {
  ...
  // The interrupted notice takes the same slot: one turn/end carries exactly
  // one reason, so the two notices never describe the same turn.
  interruptedNotice: 0.05,
