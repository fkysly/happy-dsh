...
被服务重启打断的轮次沿用同样的琥珀色提示：说明上一轮被中断，并提供模型所需的「继续」动作，它直接把该文本发给会话，不改动 composer 的草稿。
...
在触屏上，消息操作行中的每个按钮（包括由插槽提供的按钮）保持绘制的 28px 尺寸，但通过一层居中的不可见区域在至少 44 × 44 px 范围内响应点按；行内间距加倍到 16px，相邻区域不会重叠。
