...
在触屏上，`Button` 与 `Switch` 保持绘制尺寸，但通过一层居中的不可见区域在至少 44 × 44 px 范围内响应点按：`Button` 只向垂直方向扩展，因此与其他控件并排的按钮不会侵入邻居的宽度；`Switch` 两个方向都扩展，因为它是该行唯一的控件，而它覆盖到的文字本身不可交互。
