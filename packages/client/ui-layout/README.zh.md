...
- **极窄窗口**——侧栏收起时左侧仍保留 56px 控制栏，中栏仍可能小于 400px；只有展开的侧栏会切换为覆盖式抽屉。
- **覆盖式抽屉只在切换 Session 时自动收起**——点击中栏当前已显示的 Session，或在抽屉内选择全局面板，抽屉都会保持打开，需点击遮罩或使用侧栏自身的收起控件关闭。
