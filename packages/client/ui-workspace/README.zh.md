...
分组和平铺视图都先显示置顶 Session，再显示普通 Session。**最近更新**在各分区内严格按最近一次用户提示词或 steering（中途引导）时间降序排列，置顶时间不影响它。**手动排序**使用同一条完整 Session 序列中的相对位置，包括隐藏的归档项。返回最近更新会丢弃手动布局，再次进入手动排序时冻结当时的时间顺序。浏览器默认采用最近更新，并在重新加载后记住所选模式。**需要你优先**把等待操作者的会话排在前面：待审批、待审计划或待回答的排最前，其次是已完成但提醒尚未确认的回合，再次是运行中的会话，各组内部仍按最近更新排序。它回答的是「哪个任务卡在我这里」而不是「哪个最近发生过」，这正是手机上的短列表需要显示的。
