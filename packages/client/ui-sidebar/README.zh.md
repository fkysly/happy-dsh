...
在触屏上，收起的轨道中每个控件保持绘制的 36px 尺寸，但通过一层居中的不可见区域在至少 44 × 44 px 范围内响应点按；轨道的各行不再裁剪，好让这层区域伸出去。展开时头部的收起按钮在垂直方向获得同样的区域，因为它所在的品牌行在水平方向仍会裁剪。

展开的抽屉则把自身的控件——品牌行、新建会话、各面板行以及头部收起按钮——直接绘制到 44 点的下限。它们都只在透明背景上画一个图标或标签，所以放大后的方框在静止状态下不改变任何像素，也就不需要让超大的命中区域去突破会裁剪的祖先元素。
