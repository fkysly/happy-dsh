...
On a touch screen, each header button, including slot-contributed ones, keeps its drawn size but answers taps across at least 44 × 44 px through an invisible centred layer, and the spacing between header controls doubles to 16px so neighbouring layers do not overlap; buttons inside a popup rendered in a header seat keep their size so neighbouring rows never overlap. The View tabs keep their drawn size and answer taps across 44px of height, from the 10px above the tab row to 9px below it.
