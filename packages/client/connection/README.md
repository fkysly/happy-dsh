...
Every Host RPC method and WebSocket stream requires one browser session — unless the deployment sets `requireBrowserAuth: false`, which serves the UI and admits every request the trust fence accepts. Neither mode has a method-specific loopback tier. Each process mints a random launch token. `dsh-web-app` prints and opens its application URL with `?token=...`, or the bare URL when no session is required, preserving the caller's authority and mount; `frontend-static` delegates root and index requests to `ctx.connection.authorizeIndex`, which accepts that token only on `GET /`, writes an authority-bound signed cookie, and redirects to clean `./`, which drops the token and keeps the request's directory. A missing, expired, malformed, or wrong-authority cookie returns 401 before RPC dispatch. Static assets remain public. The HTTP carrier accepts no query token outside the root exchange and no Authorization-header token.

A browser that cannot open the launch URL — a Home Screen web app, whose cookies are separate from the browser's — signs in with a one-time code instead. A signed-in page creates one through `ctx.connection.createSignInCode()` (Browser), which posts to `SIGN_IN_CODE_PATH` (`/api/connection.signInCode`) behind the same fence and cookie; the route exists only when a session is required. A code is 32 random bytes, valid for ten minutes, redeemable once, and at most five unused codes exist per process; they live in memory beside the launch token, so a Connection reload keeps them and a process restart discards them. `authorizeIndex` redeems `GET /?code=...` exactly like the token — the same cookie, then `./` — and consumes a code only when the rest of the request can sign in. A refused `GET` that accepts HTML is answered with a self-contained sign-in page (no scripts or external resources, English or Chinese by `Accept-Language`) whose form submits the code; every other refused request keeps the plain-text 401.
...
Before authentication, every request still passes `src/api-request-trust.ts`. Its `Host` must be loopback or match a `trustedHosts` entry: exact on `host:port`, any port on port-less entries, both sides WHATWG-normalized. An attached `Origin` must equal that Host and `sec-fetch-site: cross-site` is refused. Malformed configured authorities fail plugin load. These checks defend DNS rebinding and cross-site browser requests; they never establish identity. A failed Host/Origin check returns 403, while a trusted but unauthenticated request returns 401 whenever a session is required. `dsh web --host 0.0.0.0` remains unsupported. Decision records: [browser request trust](../../../.agents/notes/implemented/architecture/2026-07-28-api-browser-trust-boundary.md) and [browser token authentication](../../../.agents/notes/implemented/architecture/2026-08-24-browser-token-authentication.md).
...
- **`requireBrowserAuth: false` removes the only identity check** — the trust fence then decides who is served, so every client that reaches a configured authority, and can ignore a certificate it does not trust, holds this process's own authority. Reach for it only where that reachable set is trusted, such as a single-operator tailnet or a home LAN.
