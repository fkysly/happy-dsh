...
每个 Host RPC 方法和 WebSocket 流都要求一个浏览器会话 —— 除非部署设置 `requireBrowserAuth: false`，此时 UI 直接提供，并接纳信任栅栏接受的每个请求。两种模式都不存在按方法区分的 loopback 层。每个进程生成一个随机启动令牌。`dsh-web-app` 打印并打开带 `?token=...` 的应用 URL；无需会话时则是不带令牌的 URL。两者都保留调用方的 authority 与挂载；`frontend-static` 把根路径和 index 请求交给 `ctx.connection.authorizeIndex`，后者只在 `GET /` 接受该令牌，写入绑定 authority 的签名 cookie，再重定向到干净的 `./`，移除令牌并保留请求目录。缺失、过期、畸形或 authority 不匹配的 cookie 会在 RPC 分发前得到 401。静态资源保持公开。HTTP 载体不在根路径交换之外接受 query token，也不接受 Authorization header token。

无法打开启动 URL 的浏览器 —— 例如主屏幕 Web App，它的 cookie 与浏览器分开 —— 改用一次性登录码登录。已登录的页面通过 `ctx.connection.createSignInCode()`（Browser）生成登录码，它向 `SIGN_IN_CODE_PATH`（`/api/connection.signInCode`）发 POST，经过同一信任栅栏与 cookie 校验；只有要求会话时才注册该路由。登录码为 32 字节随机数，10 分钟内有效、只能兑换一次，每个进程最多保留五个未使用的登录码；它们与启动令牌一起存放在内存中，因此 Connection 重载后仍然有效，进程重启后全部失效。`authorizeIndex` 兑换 `GET /?code=...` 的方式与令牌完全相同 —— 同样的 cookie，再重定向到 `./` —— 并且只在请求的其余部分也满足登录条件时才消耗登录码。被拒绝且接受 HTML 的 `GET` 会收到一个自包含的登录页（不含脚本与外部资源，按 `Accept-Language` 显示英文或中文），其表单提交登录码；其他被拒绝的请求仍然得到纯文本 401。
...
- **`requireBrowserAuth: false` 会移除唯一的身份校验**：此时由信任栅栏决定谁被服务，因此任何能到达已配置 authority、并且能忽略它所不信任的证书的客户端，都持有本进程自身的权限。仅当这个可达集合可信时才使用，例如单人操作的 tailnet 或家庭局域网。
